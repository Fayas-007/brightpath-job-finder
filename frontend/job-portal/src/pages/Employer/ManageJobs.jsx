    ...
    try {
      ...
      if (response.status === 200) {
        const formattedJobs = response.data?.map((job) => ({
          id: job._id,
          title: job?.title,
          company: job?.company?.companyName || job?.company?.name || "Company",
          status: job?.isClosed ? "Closed" : "Active",
          applicants: job?.applicationCount || 0,
          datePosted: formatDate(job?.createdAt),
          logo: job?.company?.avatar,
        }));
  ...
  return (
    <DashboardLayout activeMenu='manage-jobs'>
      <div className="employer-shell mb-16">
          {/* Header */}
          <div className="mb-6 employer-panel p-5 sm:p-7">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="mb-4 sm:mb-0">
                <p className="employer-kicker">Open roles</p>
                <h1 className="mt-2 text-3xl font-black text-slate-950">
                  Manage jobs
                </h1>
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Track status, review applicants, and keep listings fresh.
                </p>
              </div>

              <button
                className="employer-action whitespace-nowrap"
          ...
          {/* Filters */}
          <div className="employer-panel p-5 sm:p-6">
              ...
              {filteredAndSortedJobs.length === 0 && !isLoading ? (
                <div className="flex min-h-72 flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">
                    Clear the filters or publish a new role to start receiving candidates.
                  </p>
